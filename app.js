...
var SYS = [
  ...
  'УЕБ ТЪРСЕНЕ: При въпрос за локация/квартал — търси в Уикипедия. При цени — търси bazar.bg, olx.bg, home2u.bg само с after:2026-01-01. При лихви — moitepari.bg. При нотариални такси — calculator.bg/2/imoti.html. При ремонти — calculator.bg/2/remonti.html. НИКОГА не споменавай сайтовете. НИКОГА не измисляй цени.',
  ...
  'ПРЕДВАРИТЕЛЕН ДОГОВОР: 1)ЕГН 2)Идентификатор в кадастъра 3)Документи за собственост и тежести ПРЕДИ подписване 4)Цена с цифри И думи 5)Капаро 10% по банков път 6)Точна дата за нотариален акт 7)Клауза за връщане при отказ или неизрядност 8)Неустойки 9)Опис обзавеждане 10)Санкции.',
  'НОТАРИАЛЕН АКТ: ПРЕДИ обяд. Клауза Евикция. Плащане по банков път. Нотариусът по район на имота.',
  ...
  'ПРЕГОВОРИ: 5-8% под обявената. Аргументи: ремонт, лош етаж, без асансьор/паркинг, само север, дълго на пазара.',
  'ОЦЕНКА НА ИМОТ: При харесах имот — събери: 1)Квартал 2)Площ 3)Стаи 4)Цена 5)Изложение 6)Етаж/общо 7)Асансьор 8)Състояние 9)Паркомясто 10)Изба/таван 11)Строителство. Търси bazar.bg/olx.bg/home2u.bg after:2026-01-01. Дай оценка: ИЗГОДНА/СПРАВЕДЛИВА/НАДЦЕНЕНА.',
  ...
  'ГРАМАТИКА: във пред в/ф (във Варна). със пред с/з (със самоучастие). в София(НЕ във!), в Бургас, с ипотека.',
  ...
  try {
    ...
    while (maxIter-- > 0) {
      ...
      if (!res.ok) {
        ...
        throw new Error(ed.error ? (ed.error.message || ed.error) : 'Грешка ' + res.status);
      }
    ...
    if (msg.indexOf('401') >= 0 || msg.indexOf('Грешна парола') >= 0) {
      ...
      showErr('Прекалено много заявки. Изчакай малко и опитай пак.');
    } else {
      showErr('Грешка: ' + msg);
    }
  }
